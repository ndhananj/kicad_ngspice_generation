...
test("renders the fixture corpus and switches source tabs", async ({ page }) => {
  ...
  await expect(page.locator("#source-code")).toContainText("\\title{Fixture RC Low-pass}");
  await expect(page.locator("#document-fallback")).toBeVisible();
  await expect(page.locator("#document-fallback-title")).toHaveText("Inline report preview");
  ...
  await expect(page.locator("#document-fallback-title")).toHaveText("Report preview unavailable");
  await expect(page.locator("#document-fallback-copy")).toContainText("does not include report.tex");
});
...
test("matches the report fallback visual baseline on mobile", async ({ page }) => {
  await stabilizeFonts(page);
  await page.setViewportSize({ width: 430, height: 932 });
  await page.goto("./?test=1");
  await page.locator('[data-example-id="rc_highpass"]').click();
  await page.getByRole("tab", { name: "report.tex" }).click();
  await expect(page).toHaveScreenshot("report-fallback-mobile.png");
});
